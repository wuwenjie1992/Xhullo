...
public class XhulooActivity_MEM extends Activity{
	...
	public void onCreate(Bundle savedInstanceState) {    
		...
		am.getMemoryInfo(mf);//得到系统的内存情况
		 ...
		 List<RunningAppProcessInfo> procList = am.getRunningAppProcesses();//获得正在运行进程信息，Returns a list
		 
		 for(int i = 0; i<procList.size();i++){
			procinfo.append(procList.get(i).processName)//进程名
				.append("\t\t"+"pid:"). append(procList.get(i).pid)//进程号
				.append("\t"+"uid:").append(procList.get(i).uid).append("\n");//使用者号
		 }
