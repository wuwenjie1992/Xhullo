...
public class XhulooActivity_audioRecord extends Activity {
	...
	protected void startRecording() throws IOException {
		mrec.setAudioSource(MediaRecorder.AudioSource.MIC); // 设置音频源为Microphone
															// audio source
		mrec.setOutputFormat(MediaRecorder.OutputFormat.RAW_AMR); // 媒体录制
																	// 输出格式AMR
		mrec.setAudioEncoder(MediaRecorder.AudioEncoder.AMR_NB); // 音频编码器AMR
																	// (Narrowband窄频带)
		if (audiofile == null) {
			File sampleDir = Environment.getExternalStorageDirectory();// 获得SD卡的PATH
			try {
				audiofile = File
						.createTempFile("wuwenjie录音", ".amr", sampleDir);// 创建临时文件
			} catch (IOException e) {
				Log.e(TAG, "an error occurs when writing the file");
				return;
			}
		}// fi
	...
	protected void Broadcastaudiofile() {
		...
		values.put(MediaStore.Audio.Media.TITLE, "audio" + audiofile.getName());// title
																				// of
																				// the
																				// content
		values.put(MediaStore.Audio.Media.DATE_ADDED, (int) (current / 1000));
		values.put(MediaStore.Audio.Media.MIME_TYPE, "audio/amr"); // The MIME
																	// type of
																	// the file
		values.put(MediaStore.Audio.Media.DATA, audiofile.getAbsolutePath());// data
																				// stream
																				// for
																				// the
																				// file
		...
		Uri newUri = contentResolver.insert(base, values);// 表中插入一行,insert(Uri
															// url,ContentValues
															// values)
															// url The URL of
															// the table to
	...
	public void return_toast(String show) {
		Toast.makeText(XhulooActivity_audioRecord.this, show,
